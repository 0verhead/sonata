...
export async function fetchTicketsViaMcp(
  ...
  const notionUrl = viewId ? `https://notion.so/${boardId}?v=${viewId}` : boardId;
...
`.trim();
  ...
  const result = await runOpenCodeCli(prompt, { cwd, timeoutMs: 180_000 });
  ...
  if (totalMatch) {
    const reportedTotal = Number.parseInt(totalMatch[1], 10);
...
`.trim();
  ...
  const result = await runOpenCodeCli(prompt, { cwd, timeoutMs: 60_000 });
  ...
  if (output.includes('HAS_PRD|')) {
    ...
    const match = output.match(
      /HAS_PRD\|([a-f0-9]{32}|[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/i
    );
    if (match) {
 ...
 */
export async function fetchPageContentViaMcp(pageId: string, cwd: string): Promise<string | null> {
...
`.trim();
  ...
  const result = await runOpenCodeCli(prompt, { cwd, timeoutMs: 120_000 });
...
export async function createPrdViaMcp(
  ...
  const escapedContent = prdContent.replaceAll('`', '\\`').replaceAll('$', String.raw`\$`);
...
export async function updatePrdViaMcp(
  ...
  const escapedContent = newContent.replaceAll('`', '\\`').replaceAll('$', String.raw`\$`);
...
export async function fetchReadyTicketsViaMcp(
  ...
  const allTickets = await fetchTicketsViaMcp(
    boardId,
    statusColumn,
    cwd,
    includeInProgress,
...
`.trim();
  ...
  const result = await runOpenCodeCli(prompt, { cwd, timeoutMs: 300_000 });
