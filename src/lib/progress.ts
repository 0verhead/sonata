 ...
 */
export function initProgress(cwd: string = process.cwd(), taskFile?: string): void {
  const progressPath = getProgressPath(cwd);
 ...
 */
export function appendProgress(entry: ProgressEntry, cwd: string = process.cwd()): void {
  const progressPath = getProgressPath(cwd);
...
`;
  ...
  fs.appendFileSync(progressPath, entryText, 'utf8');
}
...
`;
  ...
  fs.appendFileSync(progressPath, completion, 'utf8');
}
